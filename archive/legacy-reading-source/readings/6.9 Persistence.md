...
This is the primary reading for Lecture 6.9 in COMPSCI 326 Web Programming. In this chapter, you will learn what persistence really means, why it is more than “saving to a file,” and how to design Node/TypeScript systems that preserve correctness when [[Appendix A Glossary#Process|processes]] restart, requests overlap, and failures happen at inconvenient times.
...
Programs end. Node [[Appendix A Glossary#Process|processes]] crash. laptops sleep, reboot, and lose in-memory state. Yet users expect that when they come back tomorrow, their entries, carts, projects, and settings are still there.
...
> data is persistent if it survives [[Appendix A Glossary#Process|process]] restart and can be retrieved in a later execution.
...
- **Volatile state:** disappears when [[Appendix A Glossary#Process|process]] or machine state resets (e.g., in-memory objects).
...
Request-lifetime state exists only while handling one HTTP request. Session-lifetime state may span multiple requests for one user. Application-lifetime state survives across requests while the [[Appendix A Glossary#Process|process]] stays alive, but disappears when the [[Appendix A Glossary#Process|process]] exits. Long-lived state survives those boundaries and is the main target of persistence.
...
![[6.9-storage-hierarchy.svg|1200]]
...
This module-level `count` value persists only while the [[Appendix A Glossary#Process|process]] is alive. If the server restarts, the value resets to zero. That is not a bug in TypeScript or Node; it is exactly how memory works.
...
In this example, `Promise<void>` means the function is asynchronous and returns a Promise that resolves with no value (it performs side effects, but does not return data). `Promise<T | null>` means the Promise resolves to either a value of type `T` (successfully read/parsed data) or `null` (no usable value found). See [[Appendix B Code Reference#Promise|Promise]].

Even this small helper expresses two important ideas. First, persistence requires serialization/deserialization ([[Appendix B Code Reference#JSON.stringify|JSON.stringify]] and [[Appendix B Code Reference#JSON.parse|JSON.parse]]). Second, writes should be designed with failure in mind (temp file then rename) instead of assuming perfect conditions. We will explain why we use [[Appendix B Code Reference#fs.rename|fs.rename]] to rename the file shortly.
...
You should observe the in-memory count restarting from `0` each [[Appendix A Glossary#Process|process]] run, while the file-backed count continues to increase across runs.
...
![[images/6.9-race-condition.svg|1200]]

This code is intentionally race-prone. The method reads `quantity`, waits (via [[Appendix B Code Reference#setTimeout|setTimeout]]), then writes `current + 1`. If two requests run this method at nearly the same time, both can read the same initial value and both write the same incremented result. One increment is effectively lost.

The key lesson is that race conditions are not only a “distributed systems” issue. They can occur inside one server [[Appendix A Glossary#Process|process]] whenever asynchronous operations interleave.
...
A logically expected result might be `2`, but you may observe `1` due to interleaving. The concurrent launch in this demo uses [[Appendix B Code Reference#Promise.all|Promise.all]]. That is the lost update phenomenon in miniature.
...
This class stores entries in a [[Appendix B Code Reference#Map|Map<string, Entry>]]. It is simple, fast, and useful for unit tests and teaching. It also resets on [[Appendix A Glossary#Process|process]] restart.

There are useful details to notice:

- IDs are generated on write ([[Appendix B Code Reference#randomUUID|randomUUID()]]), keeping identity creation near persistence operations.
...
The in-memory count reflects only current [[Appendix A Glossary#Process|process]] execution. The file-backed count grows across executions because it crosses runtime boundaries.
...
- Restart tests verifying data survival across [[Appendix A Glossary#Process|process]] restarts.

The restart test is especially important because it validates the property students often assume instead of proving.

A minimal restart-style strategy is: write canonical record, stop [[Appendix A Glossary#Process|process]], start fresh [[Appendix A Glossary#Process|process]], read record, assert existence and invariant validity.
...
3. **Restart:** fully stop the app, then launch a new [[Appendix A Glossary#Process|process]].
...
Notice what is missing from route handlers: [[Appendix B Code Reference#fs.readFile|fs.readFile]], [[Appendix B Code Reference#fs.writeFile|fs.writeFile]], or direct database calls. That is intentional. The route handles HTTP and rendering concerns. The repository handles persistence concerns.

This layout makes HTMX integration clean. `POST /entries` can return a partial list fragment for `hx-target="#entries"` while keeping persistence logic in one place. The route primitives shown here are [[Appendix B Code Reference#express.Router|express.Router]] and [[Appendix B Code Reference#res.render|res.render]].
...
5. Restart server [[Appendix A Glossary#Process|process]] and re-read state.
...
- [[Appendix B Code Reference#6.9 Persistence|Appendix B Code Reference (6.9 Persistence)]]

## Version Note

This reading targets the COMPSCI 326 lecture 6.9 examples and assumes modern Node.js with TypeScript execution via [[Appendix B Code Reference#tsx|tsx]] for local demos. Exact CLI output and generated `tsconfig.json` defaults may differ slightly by Node/npm/TypeScript version, but the persistence concepts and repository patterns in this chapter are the canonical course scope.
